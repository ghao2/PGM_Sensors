...
class Mean_vector:
        ...
        for column_index in range(0, self.testing_set_width):
            ...
            if column_index < self.testing_set_width-1:
                ...
                for sensor_index in range(0, self.data_height):
                    ...
                    current_variance=self.local_variance_for_sensors_temp[sensor_index, column_index];

                    var_next_new_sensor=self.global_variance_for_sensors_temp[sensor_index]+\
                                        beta_structure_square*current_variance;
        ...
        for column_index in range(0, self.testing_set_width):
            ...
            if column_index < self.testing_set_width-1:
                ...
                for sensor_index in range(0, self.data_height):
                    ...
                    current_variance=self.local_variance_for_sensors_humidity[sensor_index, column_index];
    ...
    def learning_structure_temp(self):
        ...
        #column number is 3, the columns are for beta0, beta1
        self.structure_matrix_temp=np.zeros([self.data_height, 2]);
        ...
        for sensor_index in range(0, self.data_height):
            ...
            #at step 1
            learning_matrix=np.zeros([self.trainning_set_width-1, 2]);

            learning_matrix[:,0]=self.trainning_temp[sensor_index, 0:-1];
            learning_matrix[:,1]=self.trainning_temp[sensor_index, 1:];
    ...
    def learning_structure_humidity(self):
        ...
        #structure must include intercept of the model, put it at the 0 position of a row
        #column number is 3, the columns are for beta0, beta1
        self.structure_matrix_humidity=np.zeros([self.data_height, 2]);
        ...
        for sensor_index in range(0, self.data_height):
            ...
            learning_matrix[:,0]=self.trainning_humidity[sensor_index, 0:-1];
            learning_matrix[:,1]=self.trainning_humidity[sensor_index, 1:];
        ...
        for column_index in range(1, self.testing_set_width):
            ...
            for row_index in range(0, self.data_height):
                ...
                if self.indicator_matrix_for_window[row_index, column_index]==0:
                    ...
                    #initialized input_x
                    input_x=self.prediction_temp[row_index, column_index-1];

                    #because only one input x, which is a float, we have to change it into an array.
                    input_x=np.array([input_x]);
        ...
        for column_index in range(1, self.testing_set_width):
            ...
            for row_index in range(0, self.data_height):
                ...
                if self.indicator_matrix_for_window[row_index, column_index]==0:
                    ...
                    #initialized input_x
                    input_x=self.prediction_humidity[row_index, column_index-1];
